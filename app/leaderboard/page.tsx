...
export default async function LeaderboardPage() {
  ...
  const exactByUser = await prisma.prediction.groupBy({
    by: ["userId"],
    where: { pointsEarned: 3 },
    _count: { id: true },
  });

  const statMap = Object.fromEntries(stats.map((s) => [s.userId, { total: s._count.id }]));
  const exactMap = Object.fromEntries(exactByUser.map((s) => [s.userId, s._count.id]));

  const ranked = users.map((u, i) => ({
    ...u,
    rank: i + 1,
    predictionsCount: statMap[u.id]?.total ?? 0,
    exactResults: exactMap[u.id] ?? 0,
  }));
          ...
          {[ranked[1], ranked[0], ranked[2]].map((p, idx) => {
            ...
            return (
              <div key={p.id} className={`card ${colors[idx]} flex flex-col items-center justify-end ${heights[idx]} border-2 relative`}>
                {podiumRank === 1 && <span className="absolute -top-3 text-2xl">👑</span>}
                <div className="text-center">
                  <div className="font-bold text-white text-sm truncate max-w-[80px]">{p.username}</div>
                  <div className={`text-lg font-black ${textColors[idx]}`}>{p.totalPoints} pts</div>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-white/10">
              ...
              <th className="text-right text-xs text-white/40 font-medium px-4 py-3">Pronóst.</th>
          ...
          <tbody>
            {ranked.map((p) => {
              ...
              return (
                <tr key={p.id} className={`border-b border-white/5 last:border-0 ${isMe ? "bg-blue-600/10" : "hover:bg-white/5"}`}>
                  <td className="px-4 py-3">
                    ...
                    {p.rank > 3 && <span className="text-white/40 text-sm font-mono">{p.rank}</span>}
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3 text-right text-white/60 text-sm">{p.predictionsCount}</td>
                  <td className="px-4 py-3 text-right text-gold-400 font-medium text-sm">{p.exactResults}</td>
                  <td className="px-4 py-3 text-right">
                    <span className={`font-bold text-lg ${isMe ? "text-blue-300" : "text-white"}`}>{p.totalPoints}</span>
                  </td>
                </tr>
              );
            })}
            {ranked.length === 0 && (
              <tr><td colSpan={5} className="text-center py-8 text-white/40">Todavía nadie anotó puntos</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      <div className="card">
        ...
        <div className="flex flex-wrap gap-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-white/50">Resultado exacto (ej: 2-1 acertado)</span>
          </div>
          <div className="flex items-center gap-2">
