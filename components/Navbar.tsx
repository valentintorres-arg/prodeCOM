...
export default function Navbar() {
  ...
  useEffect(() => {
    fetch("/api/auth/me")
      .then((r) => r.json())
      .then(({ user }) => { if (user) setUser(user); else setUser(null); })
      .catch(() => setUser(null));
  }, [pathname]);
  ...
  return (
    <nav className="border-b border-white/10 bg-navy-900/80 backdrop-blur-sm sticky top-0 z-50 pt-safe">
        ...
        <div className="flex items-center justify-between h-14">
          ...
          <div className="hidden sm:flex items-center gap-1">
            {navLinks.map(({ href, label, icon: Icon }) => (
              <Link key={href} href={href}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
                  pathname === href ? "bg-blue-600/20 text-blue-400" : "text-white/60 hover:text-white hover:bg-white/10"
                }`}>
            ...
            {user?.isAdmin && (
              <Link href="/admin"
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
                  pathname === "/admin" ? "bg-amber-600/20 text-amber-400" : "text-white/60 hover:text-white hover:bg-white/10"
                }`}>
            ...
            <div className="relative">
              <button onClick={() => setMenuOpen(!menuOpen)}
                ...
                <div className="hidden sm:block text-right">
                  ...
                  <div className="text-xs text-gold-400 font-bold">{user.totalPoints} pts</div>
                </div>
              </button>
